  ...
  const getFriendlyErrorMessage = (error: any) => {
    ...
    switch (code) {
      ...
      case "auth/weak-password":
        return "Your password must be at least 8 characters and secure.";
  ...
  const nextStep = () => {
    if (step === 1) {
      ...
      if (!validateAge(birthdate)) {
        setError("You must be at least 13 years old to use LanTrack.");
        return;
      }
      setError(null);
      setStep(3);
      ...
      >
        <Card className="p-8 md:p-10 shadow-2xl shadow-indigo-500/5 border border-white/20 dark:border-white/5 backdrop-blur-xl bg-white/80 dark:bg-zinc-900/80">
          <div className="text-center mb-8">
            ...
            >
              <UserPlus className="w-8 h-8 -rotate-12" />
            </motion.div>
            <h1 className="text-3xl font-bold tracking-tight mb-2">
              Create Account
            </h1>
            <p className="text-zinc-500 dark:text-zinc-400">
              Join LanTrack to track your hours
            </p>
          </div>
                ...
                >
                  <div>
                    ...
                    <div className="relative group">
                      <User className="w-5 h-5 absolute left-3.5 top-1/2 -translate-y-1/2 text-zinc-400 group-focus-within:text-indigo-500 transition-colors" />
                      <input
                        type="text"
                        required
                        value={name}
                  ...
                  <Button type="button" onClick={nextStep} className="w-full py-3.5 rounded-2xl text-base font-semibold">
                    Next
                  </Button>
                </motion.div>
              )}
                  ...
                  <div>
                    ...
                    <div className="relative group">
                      <input
                        ...
                        className="w-full px-4 py-3.5 rounded-2xl bg-white dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 focus:ring-4 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all shadow-sm"
                      />
                    </div>
                    <p className="text-xs text-zinc-500 mt-1 ml-1">You must be at least 13 years old.</p>
                  </div>
